...
StateSlaveHandler::StateSlaveHandler(StateSlave *slave) throw()
    :  stateSlave(slave)
    ,  QueryServerHandler<CommStateRequest, CommStateResponse>(slave->query_server)
{  }
...
void StateSlaveHandler::handleQuery(const QueryId &id, const CommStateRequest& request) throw()
{
  ...
  switch(request.getCommand())
  {
    case STATE_CMD_SET_STATE:
      ...
      StateSlave::hndSetMainState(stateSlave, dynamic_cast<SeRoNet::OPCUA::Server::QueryServer<CommStateRequest, CommStateResponse>*>(server), id, mainstate);
      break;
    ...
    default:
      std::cout << "<StateSlaveHandler> unknown command\n" << std::endl;
      break;
  };
