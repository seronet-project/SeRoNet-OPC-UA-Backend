...
class DisableOpcuaServerVisitor :
 ...
 public:
  ...
  explicit DisableOpcuaServerVisitor(const open62541::UA_NodeId &parent)
      : m_parent(parent) {}
  ...
  void visit(SeRoNet::CommunicationObjects::Description::ComplexType *complexObject) override {
    ...
    DisableOpcuaServerVisitor visitor(ownNodeId);
  ...
  void visit(SeRoNet::CommunicationObjects::Description::ElementPrimitive<bool> *el) override {
    ...
    retVal =
        UA_Server_writeAccessLevel(SeRoNet::OPCUA::Server::OpcUaServer::instance().getServer(), *myNodeId.NodeId, 0);
...
PushServerDisabler::PushServerDisabler(
  ...
  DisableOpcuaServerVisitor visitor(parent);
