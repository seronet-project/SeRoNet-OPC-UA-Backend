...
namespace SeRoNet {
	namespace OPCUA {
		namespace Client {
			...
			private:
				friend void methodCalledCallback<T_RETURN>(UA_Client *client, void *userdata, UA_UInt32 requestId, const void *response);
			...
			template<typename T_RETURN>
			inline void AsyncAnswerMethod<T_RETURN>::methodCalled_callback(UA_Client * client, void * userdata, UA_UInt32 requestId, const void * response)
			{
				...
				if (callResponse->resultsSize > 0) {
					if (callResponse->results[0].outputArgumentsSize > 0) {
						outArgs.reset(new open62541::UA_ArrayOfVariant(callResponse->results[0].outputArguments, callResponse->results[0].outputArgumentsSize));
					}
				}
