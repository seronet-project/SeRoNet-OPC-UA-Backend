...
class AsyncSubscriptionOpcUa : public AsyncSubscriptionArrayBuffer<T_DATATYPE> {
 public:
  ...
  typedef std::function<void(const T_DATATYPE&)> onDataCallback_t;
  void setOnDataCallback(onDataCallback_t onDataCallback = onDataCallback_t());
 protected:
 ...
 private:
  onDataCallback_t m_onDataCallback;
...
inline UA_StatusCode AsyncSubscriptionOpcUa<T_DATATYPE>::subscribe(std::vector<UA_NodeId> nodeIds) {
  ...
  for (auto &nodeId : nodeIds) {
    UA_MonitoredItemCreateRequest monItemRequest =
        UA_MonitoredItemCreateRequest_default(UA_NODEID_NULL);
    UA_NodeId_copy(&nodeId, &(monItemRequest.itemToMonitor.nodeId));
    monItemRequest.monitoringMode = UA_MONITORINGMODE_REPORTING;
    monItemRequest.requestedParameters.queueSize = 3;
    /// \todo set elsewhere
    monItemRequest.requestedParameters.samplingInterval = 50;
    ...
    auto dataChangeFilter = UA_DataChangeFilter_new();
    UA_DataChangeFilter_init(dataChangeFilter);
    dataChangeFilter->trigger = UA_DataChangeTrigger::UA_DATACHANGETRIGGER_STATUSVALUETIMESTAMP;
    dataChangeFilter->deadbandValue = 0.0;
    dataChangeFilter->deadbandType = 0;
    monItemRequest.requestedParameters.filter.content.decoded.data = dataChangeFilter;
...
inline void AsyncSubscriptionOpcUa<T_DATATYPE>::valueChanged(UA_UInt32 monId, UA_DataValue *value) {
  ...
  if (!value->hasValue) {
    std::cout << "No value in publish" << std::endl;
    return;
  }
...
template<typename T_DATATYPE>
void AsyncSubscriptionOpcUa<T_DATATYPE>::addData(const T_DATATYPE &newData) {
  AsyncSubscriptionArrayBuffer<T_DATATYPE>::addData(newData);

  if(m_onDataCallback)
  {
    m_onDataCallback(newData);
  }
}

template<typename T_DATATYPE>
void AsyncSubscriptionOpcUa<T_DATATYPE>::setOnDataCallback(onDataCallback_t onDataCallback) {
  m_onDataCallback = onDataCallback;
  /// \TODO use return type to track the callback and prevent handle when the callback is removed before this class
}
