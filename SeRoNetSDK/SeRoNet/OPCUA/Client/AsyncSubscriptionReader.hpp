	...
	namespace OPCUA {
		namespace Client {
			...
			public:
				...
				explicit AsyncSubscriptionReader(std::shared_ptr<AsyncSubscription<T_DATATYPE>> pSubscription) : m_pSubscription(pSubscription) {}
			...
			void AsyncSubscriptionReader<T_DATATYPE>::seek(std::int64_t offset, AsyncSubscriptionReader::SEEK_REF referece)
			{
				...
				{
					case REF_FIRST:
						newRequestedCounter = offset;
						break;
					case REF_CURRENT:
						newRequestedCounter = m_dataFetchCounter + offset;
						break;
					case REF_END:
						newRequestedCounter = endCounter + offset;
						break;
					default:
						///@todo error handling
